...
export class TwitterService {
  ...
  async addMediaAccount(mediaAccount: string, startTime: string): Promise<void> {
      try {
  ...
  async removeMediaAccount(mediaAccount: string): Promise<void> {
      try {
          ...
          let twitter = await this.twitterRepository.findOne({ where: { account: mediaAccount } });
  ...
  async getUserId(mediaAccount: string): Promise<string> {
    ...
    try {
      ...
      this.logger.warn(`Failed to get user id by media account: ${error.message}`);
      throw new Error('Media account not found');
    }
  }
  ...
  async fetchTweets() {
      ...
      for (const account of accounts) {
          try {
              ...
              this.logger.log(`Fetching tweets for account: ${account.account}, at ${fetchData?.start_time}`);

              const tweets = await this.getTweets(account.uid, fetchData.start_time);
              this.logger.log(`Fetched ${tweets.length} tweets for account ${account.account}`);
              ...
              this.logger.log(`Saving ${fetchEntities.length} tweets for account ${account.account}`);
              ...
              this.logger.error(`Failed to fetch tweets for account ${account.account}: ${error.message}`);
          }
      }
  }
              ...
              for (const item of resp) {
                  ...
                  if (entryId.includes('tweet')) {
                      if (item.content?.itemContent) {
                          ...
                          this.logger.log(`tweet created time: ${created_at}, tweet id: ${tweet_id}`);

                          const tweetTime = new Date(created_at);

                          if (tweetTime < startTime) {
                              this.logger.log("tweet created time is before start time, stop getting more tweets");
                              return tweets;
                          }
  ...
  async getTweetsByAccount(mediaAccount: string): Promise<Array<Tweet>> {
      ...
      const account = await this.twitterRepository.findOne({ where: { account: mediaAccount } });
      if (!account) {
          throw new Error('Media account not found');
      }
